...
graph LR
	...
	mainwindow-->Access
	Administrator-->addInfo
	Access-->addAccessAction
```
...
**globaluse.h和globaluse.cpp文件中定义和实现了一些需要全局使用的数据结构/函数。**

```mermaid
graph RL
	Administrator--use-->globaldata/function
	Access--use-->globaldata/function
	addInfo--use-->globaldata/function
	addAccessAction--use-->globaldata/function
```



关于这些类和数据结构，以下将进行更为详细的介绍。
...
```c++
//globaluse.cpp
vector <permitInfo> permitInfoVector;		//用于存放准入信息
```
...
```c++
//globaluse.cpp
vector <moveInfo> moveInfoVector;			//用于存放当前出入信息
```
